...
## This is a modified version of bitsandbytes/functional.py with torch.compile compatabilies
## Replace this file with the existing file in the library as per the instructions in the README.md
...
@torch.library.custom_op("mylib::dynamoCompatibleIsOnGPU", mutates_args=())
def is_on_gpu(tensors: List[torch.Tensor]) -> torch.Tensor:
...
@is_on_gpu.register_fake
def _(tensors: List[torch.Tensor]) -> torch.Tensor:
    return tensors[0].clone()
...
def dequantize_blockwise(
    ...
    if A.device.type != "cpu":
        ...
        with _cuda_device_of(A):
            dynamoCompatibleDequantize(A, quant_state.code, absmax, out, quant_state.blocksize)
    else:
...
@torch.library.custom_op("mylib::dynamoCompatibleDequantize4bit", mutates_args={"out"})
def dynamoCompatibleDequantize4bit(A: torch.Tensor,
    absmax: Optional[torch.Tensor] = None,
    out: Optional[torch.Tensor] = None,
    qblocksize: int = 4096,
    n: int = 64,
...
def dequantize_4bit(
    ...
    with _cuda_device_of(A):
        dynamoCompatibleDequantize4bit(A, absmax, out, quant_state.blocksize, n, quant_state.quant_type)
